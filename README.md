# Fur-Dev PurPur 💜

A vibrant and cozy purple-tinted dark VS Code theme designed for coding comfort, high contrast, and clean readability!

## Features

* **Eliminated the Purple Haze:** A carefully balanced color palette where every token has its own distinct color family so nothing blends together.
* **Distinct Color Coding:** 
  * 🟡 **Gold/Yellow** for Classes & Inheritance
  * 🔵 **Sky Blue** for Functions & Methods
  * 🟢 **Mint Green** for Strings
  * 🟠 **Orange** for Parameters, Attributes, & Numbers
  * ❄️ **Ice Blue** for Variables
  * 🔴 **Sharp Red** for Invalid tokens and errors
* **Cozy Dark Background:** Built on a deep `#140f1a` background that is easy on the eyes during late-night coding sessions.

---

## Installation

### Method: Install via VSIX
1. Head over to the [Releases]([https://github.com/katt-dev/Katt-Dev-PurPur/releases](https://github.com/katt-dev/Fur-Dev-PurPur/releases/tag/v1.0.0)  page of this repository.
2. Download the latest `.vsix` file (`CatGirlUmi-Fur-Dev-PurPur-0.0.1.vsix`).
3. Open Visual Studio Code.
4. Go to the **Extensions** sidebar (`Ctrl+Shift+X` or `Cmd+Shift+X`).
5. Click the **`...` (Views and More Actions)** button at the top right of the extensions panel and select **Install from VSIX...**.
6. Choose the downloaded file.
7. Press `Ctrl+K Ctrl+T` (or `Cmd+K Cmd+T`), select **Fur-Dev PurPur**, and enjoy!

---
*Made with 💜 by Fur-Cord*
